...
#getSymbols("LNKD", from = '1900-01-01')
#getSymbols("LNKD")
print(head(LNKD))
print(tail(LNKD))
chartSeries(LNKD, theme="white")

#trainset <- window(LNKD, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
trainsetraw <- window(LNKD, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
print(ncol(trainsetraw)-1)
print(trainsetraw[,1:ncol(trainsetraw)-1])
trainset <- na.omit(trainsetraw[,1:ncol(trainsetraw)-1])
print(trainset)

#LNKD_Subset <- window(LNKD, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
...
for (i in 1: rows) {
	...
	#diff = (abs ((pred - actual)/ actual))[1,]$LNKD.Close
