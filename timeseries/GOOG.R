...
#getSymbols("GOOG", src = "google")
#getSymbols("GOOG", from = '1900-01-01')
#getSymbols("GOOG")
print(head(GOOG))
print(tail(GOOG))
chartSeries(GOOG, theme="white")

#trainset <- window(GOOG, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
trainsetraw <- window(GOOG, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
print(ncol(trainsetraw)-1)
print(trainsetraw[,1:ncol(trainsetraw)-1])
trainset <- na.omit(trainsetraw[,1:ncol(trainsetraw)-1])
print(trainset)

#GOOG_Subset <- window(GOOG, start = as.Date("1900-01-01"), end = as.Date("2013-04-01"))
...
for (i in 1: rows) {
	...
	#diff = (abs ((pred - actual)/ actual))[1,]$GOOG.Close
