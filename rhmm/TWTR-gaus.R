...
# scatter plot
postscript('TWTR-gaus.eps')
TWTR_Predict <- cbind(trainset$TWTR.Close, VitPath$states)
#TWTR_Predict <- cbind(TWTR_Subset$TWTR.Close, VitPath$states)
#print(TWTR_Subset[,4] - TWTR_Predict [,1])
...
#testset <- cbind(testset$TWTR.Close, Pred = 0)
#print(testset)
...
for (i in 1: rows) {
	...
	if(i != 0) {
		testrow <- testset[i, ]
		#print(testrow)
		todayopen <- testset$TWTR.Open[i, ]
		todayclose <- testset$TWTR.Close[i, ]
	}
	...
	diff = (abs ((pred - todayclose)/ todayclose))[1,]$TWTR.Open
	#print ("diff")
	#print (diff)
	#MAPEsum <- MAPEsum + diff$TWTR.Open
...
#chartSeries(testset[2:rows, 4], theme='white', col = 'green', name = "TWTR", legend = "Actual",
chartSeries(testset[1:rows, 1], theme= chartTheme('white', up.col = 'blue'), name = "TWTR", legend = "Actual",
...
#chartSeries(TWTR_Predict[,1], layout = layout(matrix(2:1)), # 1, 2, byrow = TRUE), #respect = TRUE), #theme="white.mono", 
#TA="addTA(TWTR_Predict[TWTR_Predict[,2]==1,1], legend = \"one day?\", on=1, col=5,pch=25);
#addTA(TWTR_Predict[TWTR_Predict[,2]==2,1],on=1,type='p',col=6,pch=24);
#addTA(TWTR_Predict[TWTR_Predict[,2]==3,1],on=1,type='p',col=7,pch=23);
#addTA(TWTR_Predict[TWTR_Predict[,2]==4,1],on=1,type='p',col=8,pch=22);
#addTA(TWTR_Predict[TWTR_Predict[,2]==5,1],on=1,type='p',col=10,pch=21)
#")
